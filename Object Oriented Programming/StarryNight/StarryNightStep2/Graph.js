class Graph {
	...
	drawGraph(ctx, value, x, y, w, h) {
		const { valueHistory, currentMaxValue } = this.getGraphData(this.title)
		...
		if (currentMaxValue === undefined) {
			this.graphData.get(this.title).currentMaxValue = targetMaxValue
		}

		const lerp = (start, end, amt) => (1 - amt) * start + amt * end
		let updatedMaxValue = this.graphData.get(this.title).currentMaxValue
		updatedMaxValue = lerp(updatedMaxValue, targetMaxValue, 0.1)
		this.graphData.get(this.title).currentMaxValue = updatedMaxValue
		...
		for (let i = 0; i < valueHistory.length; i++) {
			...
			const y = Math.min(
				Math.max(
					graphY +
						graphHeight -
						(normalizedValue / roundedMaxValue) * graphHeight,
					graphY
				),
				graphY + graphHeight
			)
		...
		if (this.higherIsBetter) {
			...
			const greenHeight =
				((roundedMaxValue - this.greenThreshold) / roundedMaxValue) *
				graphHeight
			ctx.fillRect(graphX, graphY, graphWidth, greenHeight)

			ctx.fillStyle = yellowColor
			const yellowHeight =
				((this.greenThreshold - this.yellowThreshold) / roundedMaxValue) *
				graphHeight
			...
			ctx.fillRect(
				graphX,
				graphY + greenHeight + yellowHeight,
				graphWidth,
				redHeight
			)
		} else {
			...
			ctx.fillRect(
				graphX,
				graphY + (graphHeight - greenHeight),
				graphWidth,
				greenHeight
			)
			...
			ctx.fillStyle = yellowColor
			const yellowHeight =
				((this.yellowThreshold - this.greenThreshold) / roundedMaxValue) *
				graphHeight
			ctx.fillRect(
				graphX,
				graphY + (graphHeight - greenHeight - yellowHeight),
				graphWidth,
				yellowHeight
			)
			...
			ctx.fillStyle = redColor
			const redHeight =
				((roundedMaxValue - this.yellowThreshold) / roundedMaxValue) *
				graphHeight
		...
		for (let i = 0; i <= this.labelCount; i++) {
			const value = (
				(roundedMaxValue * (this.labelCount - i)) /
				this.labelCount
